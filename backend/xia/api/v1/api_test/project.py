...
from backend.xia.common.response.response_schema import Response200, Response403
...
def get_one_project(request, pk: int) -> Any:
    try:
        _project = ApiTestProjectDao.get_project_or_404(pk)
    except Http404:
        raise Http404("未找到项目")
...
def delete_project(request, pk: int) -> Any:
    try:
        ApiTestProjectDao.delete_project(pk)
    except Http404:
        raise Http404("未找到项目")
    return Response200()


@v1_api_test_project.get('/{int:pk}/modules', response=List[GetAllApiTestModules], summary='获取单个项目所有模块',
                         auth=GetCurrentUser())
@paginate(CustomPagination)
